...
async function fetchDataStore() {
  try {
    ...
    if (res.ok) {
      ...
      if (!localStorage.getItem("medicare_doctors") && data.doctors && data.doctors.length > 0) {
        appState.doctors = data.doctors;
      }
      if (data.users && data.users.length > 0 && appState.users.length <= 3) {
        appState.users = data.users;
      }
      if (!localStorage.getItem("medicare_feedback") && data.feedback && data.feedback.length > 0) {
        appState.feedback = data.feedback;
      }
...
function loadPersistedData() {
  try {
    ...
    const savedFeedback = localStorage.getItem("medicare_feedback");
    if (savedFeedback) appState.feedback = JSON.parse(savedFeedback);

    const savedDoctors = localStorage.getItem("medicare_doctors");
    if (savedDoctors) appState.doctors = JSON.parse(savedDoctors);
...
function savePersistedData() {
  try {
    ...
    localStorage.setItem("medicare_feedback", JSON.stringify(appState.feedback));
    localStorage.setItem("medicare_doctors", JSON.stringify(appState.doctors));
    ...
    <div class="doctor-card glass-card">
        ...
        <div class="doctor-info">
            ...
            <div class="doctor-card-actions">
                <button class="btn btn-cyan-gradient ripple-btn" style="flex: 1;" onclick="openBooking(${doctor.id})">Book Consultation</button>
                <button class="btn btn-glass-secondary ripple-btn" onclick="openReviewModal(${doctor.id})" title="Leave Review for ${doctor.name}">✍️ Review</button>
            </div>
        </div>
    </div>
  `;
...
function switchAuthTab(event, tab) {
  ...
  if (loginAlert) { loginAlert.classList.add("hidden"); loginAlert.innerHTML = ""; }
  if (signupAlert) { signupAlert.classList.add("hidden"); signupAlert.innerHTML = ""; }
}
...
function handleLogin(e) {
  ...
  // Fallback demo users if array is empty
  if (!appState.users || appState.users.length === 0) {
    appState.users = [
      { id: 1, name: "John Doe", email: "john@example.com", phone: "555-0101", joinDate: "2025-01-15", password: "password123" },
      { id: 2, name: "Jane Smith", email: "jane@example.com", phone: "555-0102", joinDate: "2025-01-20", password: "password123" }
    ];
  }
  ...
  if (user) {
    ...
    if (alertEl) {
      ...
      alertEl.innerHTML = `⚠️ <strong>Invalid email or password.</strong> Please verify your credentials or create an account under Sign Up.`;
...
function handleSignup(e) {
  ...
  const confirmPassword = e.target.signupConfirmPassword ? e.target.signupConfirmPassword.value : password;
  const phone = e.target.signupPhone.value.trim();

  if (password !== confirmPassword) {
    showToast("Passwords do not match.", "warning");
    if (alertEl) {
      alertEl.className = "auth-alert warning";
      alertEl.innerHTML = `⚠️ <strong>Passwords do not match.</strong> Please make sure both password fields are identical.`;
      alertEl.classList.remove("hidden");
    }
    return;
  }
  ...
  if (appState.users.find((u) => u.email.toLowerCase() === email.toLowerCase())) {
    ...
    if (alertEl) {
      alertEl.className = "auth-alert warning";
      alertEl.innerHTML = `⚠️ <strong>An account with this email already exists.</strong> Click <strong>Login</strong> tab to sign in.`;
      alertEl.classList.remove("hidden");
    }
    return;
  }
...
function renderTestimonials() {
  ...
  if (!appState.feedback || appState.feedback.length === 0) {
    container.innerHTML = `<p class="text-center" style="padding: 2rem; color: var(--text-muted);">No patient reviews yet. Be the first to leave a review!</p>`;
    dotsContainer.innerHTML = "";
    return;
  }

  if (appState.currentTestimonialIndex >= appState.feedback.length) {
    appState.currentTestimonialIndex = 0;
  }

  container.innerHTML = appState.feedback
    .map((item, idx) => {
      const name = item.patient || item.user || "Verified Patient";
      const initials = name
        .split(" ")
        .map((n) => n.charAt(0))
        .join("")
      ...
      const stars = "⭐".repeat(fullStars) + (hasHalf ? "½" : "") + ` (${item.rating}/5)`;
      const docTag = item.doctorName ? `<div class="testimonial-doctor-tag">👨‍⚕️ Reviewed: <strong>${item.doctorName}</strong></div>` : "";
        ...
        <div class="testimonial-slide ${idx === appState.currentTestimonialIndex ? "active" : ""}">
            ...
            <div class="testimonial-author">
                ...
                <div>
                    ...
                    <span class="author-role">Verified Patient Review${dateTag}</span>
                </div>
            </div>
        </div>
      `;
...
function openReviewModal(targetDoctorId = null) {
  ...
  const doctorSelect = document.getElementById("reviewDoctorSelect");
  if (doctorSelect) {
    let optionsHTML = `<option value="">General Platform & Care Review</option>`;
    appState.doctors.forEach((doc) => {
      const isSelected = targetDoctorId && doc.id === targetDoctorId ? "selected" : "";
      optionsHTML += `<option value="${doc.id}" ${isSelected}>${doc.name} (${doc.specialty})</option>`;
    });
...
function submitReview(e) {
  ...
  const doctorSelect = document.getElementById("reviewDoctorSelect");
  const doctorIdStr = doctorSelect ? doctorSelect.value : "";
  ...
  let doctorName = null;
  if (doctorIdStr) {
    const docId = parseInt(doctorIdStr);
    const doctor = appState.doctors.find((d) => d.id === docId);
    if (doctor) {
      doctorName = `${doctor.name} (${doctor.specialty})`;
      const totalReviews = (doctor.reviews || 0) + 1;
      const currentRating = doctor.rating || 5.0;
      const newRating = Number((((currentRating * (totalReviews - 1)) + rating) / totalReviews).toFixed(1));
      doctor.reviews = totalReviews;
      doctor.rating = Math.min(5, Math.max(1, newRating));
    }
  }
  ...
  appState.feedback.unshift(newFeedback);
  appState.currentTestimonialIndex = 0;
  savePersistedData();
  renderDoctorGrid(appState.doctors);
  renderTestimonials();
